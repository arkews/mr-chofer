...
export const RegisterRideRequestSchema = z
  .object({
    passenger_id: z
      .string({ required_error: 'El pasajero es requerido' })
      .min(1, 'El pasajero es requerido'),
    pickup_location: z
      .string({ required_error: 'La ubicación de recogida es requerida' })
      .min(1, 'La ubicación de recogida es requerida'),
    destination: z
      .string({ required_error: 'La ubicación de destino es requerida' })
  ...
  .refine(
    async (data) => {
      const key =
        data.gender === 'Male' ? 'MINIMUM_MALE_FARE' : 'MINIMUM_FEMALE_FARE'
